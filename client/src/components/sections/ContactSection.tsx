  ...
  return (
    <section id="contato" className="botanical-section section-space scroll-mt-24">
      <div className="mx-auto max-w-7xl px-6 lg:px-10">
        <div className="grid gap-8 lg:grid-cols-[1fr_.96fr] lg:items-stretch">
          <div className="rounded-[2.2rem] border border-[rgb(var(--brown-rgb)/0.14)] bg-[rgb(var(--cream-rgb)/0.82)] p-8 shadow-[0_28px_70px_rgb(var(--brown-rgb)/0.12)] backdrop-blur sm:p-10 lg:p-11">
            <SectionTitle eyebrow="Venha nos visitar" title="Nos Encontre" centered={false} />

            <p className="section-body-text mt-6 max-w-lg text-[rgb(var(--brown-rgb))]">
            ...
            <div className="mt-9 grid gap-5">
              {details.map(({ icon: Icon, label, value, href }) => (
                <div
                  key={label}
                  className="flex gap-4 rounded-[1.3rem] border border-[rgb(var(--brown-rgb)/0.12)] bg-[rgb(var(--cream-rgb)/0.6)] p-4"
                >
                  ...
                  <div>
                    ...
                    {href ? (
                      <a
                        ...
                        className="section-support-text mt-1 block !text-[rgb(var(--brown-rgb))] hover:!text-[rgb(var(--terracotta-rgb))]"
                      >
                        {value}
                      </a>
                    ) : (
                      <p className="section-support-text mt-1 text-[rgb(var(--brown-rgb))]">{value}</p>
                    )}
                  </div>
                </div>
              ))}
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="mt-10 inline-flex h-[3.25rem] items-center justify-center gap-3 rounded-full bg-[rgb(var(--green-rgb))] px-7 text-xs font-bold uppercase tracking-[.14em] !text-white transition hover:bg-[rgb(var(--terracotta-rgb))]"
            >
          ...
          <div className="relative overflow-hidden rounded-[2.2rem] border border-[rgb(var(--brown-rgb)/0.18)] bg-[rgb(var(--cream-rgb)/0.56)] shadow-[0_28px_70px_rgb(var(--brown-rgb)/0.12)]">
            ...
            <iframe
              ...
              className="h-full min-h-[400px] w-full lg:min-h-[100%] [filter:sepia(16%)_saturate(72%)_hue-rotate(-8deg)_brightness(1.02)_contrast(.92)]"
            />
          </div>
        </div>
      </div>
